...
Game::Game(std::vector<Node *> cars,
           std::vector<std::vector<int>> originalCarPosition, Node *teaPot,
           Node *brokenTeaPot)
    : cars(cars), originalCarPosition(originalCarPosition), teaPot(teaPot),
      brokenTeaPot(brokenTeaPot) {
  for (auto &car : cars)
  ...
  std::shared_ptr<Material> material =
      dynamic_cast<Mesh *>(teaPot)->getMaterial();
  if (material) {
...
float Game::getMovementSpeed() { return movementSpeed; }
...
void Game::setActiveCar(int activeCar) {
  ...
  updateCarEmission(activeCar == 0 ? cars.size() - 1 : activeCar - 1,
      baseEmissionOff);
  updateCarEmission(activeCar == 7 ? 0 : activeCar + 1,
      baseEmissionOff);
}
...
void Game::moveCar(glm::vec2 direction) {
  ...
  // Assuming the orientation.y (yaw) determines whether the car is vertical or
  // horizontal Adjust the threshold angle as needed
  float thresholdAngle = 45.0f; // degrees

  glm::vec3 movementDirection;

  if (std::abs(orientation.y) < thresholdAngle ||
      std::abs(orientation.y) > (180.0f - thresholdAngle)) {
  ...
  glm::mat4 translation =
      glm::translate(glm::mat4(1.0f), movementDirection * movementSpeed);
...
void Game::biLux() {
  ...
  float glowIntensity = isGlowingUp ? (elapsedTime / glowDuration)
