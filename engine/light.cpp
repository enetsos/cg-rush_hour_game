...
LIB_API Light::Light(const int id, const std::string name,
                     const int lightNumber, const glm::vec4 ambient,
                     const glm::vec4 diffuse, const glm::vec4 specular)
    : Node{id, name}, lightNumber(lightNumber), ambient(ambient),
...
void Light::setPosition(glm::vec4 pos) { position = pos; }

glm::vec4 LIB_API Light::getPosition() { return position; }
...
int Light::getLightNumber() { return lightNumber; }

float LIB_API Light::getConstantAttenuation() { return constantAttenuation; }
float LIB_API Light::getLinearAttenuation() { return linearAttenuation; }
float LIB_API Light::getQuadraticAttenuation() { return quadraticAttenuation; }
...
void LIB_API Light::render(glm::mat4 cameraInv) {
  ...
  glLightfv(lightNumber, GL_POSITION,
            glm::value_ptr(glm::vec4(0.0f, 0.0f, 0.0f, position.w)));
}