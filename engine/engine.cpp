...
/////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
/**
 * DLL entry point. Avoid to rely on it for easier code portability (Linux
...
void LIB_API Engine::init(const char *windowName,
                          void (*keyboardCallbackApplication)(int),
  ...
  // Initialize the UI
  ui =
      new UIProjection(glutGet(GLUT_SCREEN_WIDTH), glutGet(GLUT_SCREEN_HEIGHT));
  fps = new FrameRate();
}
...
void LIB_API Engine::setCamera(Camera *camera) { this->camera = camera; }
...
Node LIB_API *Engine::loadScene(std::string fileName) {
  ...
  // free camera
  Projection *proj = new PerspectiveProjection(glutGet(GLUT_SCREEN_WIDTH),
                                               glutGet(GLUT_SCREEN_HEIGHT),
                                               45.0f, 1.0f, 1000.0f);
  Camera *camera =
      new Camera(Object::getNextId(), std::string("freeCamera"), proj);
  root->addChild(camera);

  // stationary camera
  proj = new PerspectiveProjection(glutGet(GLUT_SCREEN_WIDTH),
                                   glutGet(GLUT_SCREEN_HEIGHT), 45.0f, 1.0f,
                                   1000.0f);
  camera =
      new Camera(Object::getNextId(), std::string("stationaryCamera"), proj);
  root->addChild(camera);
...
List LIB_API Engine::getList() { return Engine::list; }

// Texture
void Engine::setTexturePath(std::string path) { Texture::setPath(path); }

// UI
UIProjection LIB_API *Engine::getUI() { return ui; }

// Vector returned with size elements
//  "width" and "height"

int LIB_API Engine::getFps() { return fps->getFps(); }