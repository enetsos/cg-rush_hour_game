...
void LIB_API Texture::setTexId(std::string file) {
  ...
  FIBITMAP *bitmap = FreeImage_Load(FreeImage_GetFileType(filePath.c_str(), 0),
                                    filePath.c_str());
  FreeImage_FlipVertical(bitmap);

  gluBuild2DMipmaps(GL_TEXTURE_2D, GL_RGBA, FreeImage_GetWidth(bitmap),
                    FreeImage_GetHeight(bitmap), GL_BGRA_EXT, GL_UNSIGNED_BYTE,
