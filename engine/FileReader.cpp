...
Node *FileReader::readFile(const char *path) {
  ...
  Material *shadow_material = new Material(
      Object::getNextId(), "shadow_material", glm::vec4(glm::vec3(0.0f), 1.0f),
      glm::vec4(glm::vec3(0.0f), 1.0f), glm::vec4(glm::vec3(0.0f), 1.0f),
      glm::vec4(glm::vec3(0.0f), 1.0f), 0.0f);
  materials.insert(std::pair<std::string, Material *>(
      shadow_material->getName(), shadow_material));
...
Node *FileReader::recursiveLoad(FILE *dat) {
  ...
  if ((OvObject::Type)chunkId == OvObject::Type::O_MATERIAL) {
    ...
    Material *material = new Material(
        Object::getNextId(), materialName_str, glm::vec4(emission, 1.0f),
        glm::vec4(albedo * 0.2f, 1.0f), glm::vec4(albedo * 0.6f, 1.0f),
        glm::vec4(albedo * 0.4f, 1.0f), (1 - sqrt(roughness)) * 128);
    materials.insert(
        std::pair<std::string, Material *>(material->getName(), material));
  ...
  case OvObject::Type::O_MESH: {
    ...
    if (materialName_str != "[none]") {
      std::shared_ptr<Material> material(
          materials.find(materialName_str)->second);
    ...
    std::shared_ptr<Material> smaterial(
        materials.find("shadow_material")->second);
    FakeShadow *shadow = new FakeShadow(
        Object::getNextId(), nodeName_str + "_shadow", smaterial, thisMesh);
    ...
    for (unsigned int l = 0; l < LODs; l++) {
      ...
      for (unsigned int c = 0; c < vertices; c++) {
        ...
        Vertex *newVertex =
            new Vertex(vertex, glm::vec3(normal.x, normal.y, normal.z));
  ...
  case OvObject::Type::O_LIGHT: {
    ...
    switch ((OvLight::Subtype)subtype) {
    case OvLight::Subtype::DIRECTIONAL: {
      thisLight = new DirectionalLight(Object::getNextId(), nodeName_str,
                                       nextLightPointer,
                                       glm::vec4(glm::vec3(color.x), 1.0f),
                                       glm::vec4(glm::vec3(color.y), 1.0f),
                                       glm::vec4(glm::vec3(color.z), 1.0f));
    } break;
    case OvLight::Subtype::OMNI: {
      thisLight =
          new PointLight(Object::getNextId(), nodeName_str, nextLightPointer,
                         glm::vec4(glm::vec3(color.x), 1.0f),
                         glm::vec4(glm::vec3(color.y), 1.0f),
                         glm::vec4(glm::vec3(color.z), 1.0f), cutoff);
    } break;
    case OvLight::Subtype::SPOT: {
      thisLight =
          new SpotLight(Object::getNextId(), nodeName_str, nextLightPointer,
                        glm::vec4(glm::vec3(color.x), 1.0f),
                        glm::vec4(glm::vec3(color.y), 1.0f),
                        glm::vec4(glm::vec3(color.z), 1.0f), cutoff, direction);
    } break;
    default:
      thisLight = new Light(Object::getNextId(), nodeName_str, nextLightPointer,
                            glm::vec4(glm::vec3(color.x), 1.0f),
                            glm::vec4(glm::vec3(color.y), 1.0f),
                            glm::vec4(glm::vec3(color.z), 1.0f));
    }
